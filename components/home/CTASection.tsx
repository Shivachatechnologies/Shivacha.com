...
export default function CTASection() {
  return (
    <section className="relative overflow-hidden py-28" style={{ background: '#060B18' }}>
      {/* Background */}
      <div className="absolute inset-0 pointer-events-none"
        style={{ background: 'radial-gradient(ellipse 70% 50% at 50% 100%, rgba(59,130,246,0.10) 0%, transparent 70%)' }} />
      <div className="absolute inset-0 dot-bg opacity-[0.25] pointer-events-none" />

      <div className="container relative z-10 text-center">
        <div className="max-w-3xl mx-auto mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full border border-white/[0.08] bg-white/[0.04] mb-8">
            <span className="w-1.5 h-1.5 rounded-full bg-[#3B82F6] animate-pulse" />
            <span className="text-[12px] font-semibold text-[rgba(240,246,255,0.45)] tracking-wide">Ready to Build</span>
          </div>
          ...
          <div className="flex flex-wrap gap-3 justify-center mb-14">
            <Link href="/contact#consultation" className="btn btn-primary text-[15px] py-3 px-7 flex items-center gap-2">
              Book Free Consultation <ArrowRight size={16} />
            </Link>
            <Link href="/products/crypto-exchange" className="btn btn-secondary text-[15px] py-3 px-7">
              Explore Products
            </Link>
          </div>
            ...
            {[
              {
                ...
                cta: 'Schedule Now', href: '/contact#schedule', ctaColor: '#60A5FA',
              },
              {
                ...
                cta: 'Send Brief', href: '/contact', ctaColor: '#A78BFA',
              },
              {
                icon: <Phone size={18} />,
                ...
                cta: '+1 800 800 0000', href: 'tel:+18008000000', ctaColor: '#34D399',
              },
            ].map(card => (
              <div key={card.title}
                ...
                <p className="text-[13px] text-[rgba(240,246,255,0.40)] leading-relaxed mb-4">{card.desc}</p>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {[
              ...
              <div key={o.country} className="text-left bg-white/[0.03] border border-white/[0.06] rounded-xl p-5">
                ...
                <p className="text-[12.5px] text-[rgba(240,246,255,0.35)] whitespace-pre-line mb-2 leading-relaxed">{o.address}</p>
                <a href={`tel:${o.phone.replace(/[\s()]/g,'')}`}
                  className="text-[12.5px] text-[rgba(240,246,255,0.35)] hover:text-[#60A5FA] transition-colors block">{o.phone}</a>
                <a href={`mailto:${o.email}`}
                  className="text-[12.5px] text-[rgba(240,246,255,0.35)] hover:text-[#60A5FA] transition-colors block">{o.email}</a>
              </div>
            ))}
          </div>
        </div>
