...
const METRICS = [
  { value: 500,  suffix: '+',    label: 'Projects Delivered',    color: '#006FEE' },
  ...
  { value: 8,    suffix: '+',    label: 'Years of Innovation',    color: '#EF4444' },
];
...
function Counter({ end, suffix, color }: { end: number; suffix: string|null; color: string }) {
  ...
  return <div ref={ref} className="text-[2.4rem] font-black tracking-tight leading-none" style={{ color }}>{display}</div>;
}

export default function StatsSection() {
  return (
    <section className="section-sm bg-[#05080F]">
      <div className="container">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-px bg-white/[0.06] rounded-2xl overflow-hidden border border-white/[0.06]">
          {METRICS.map((m) => (
            <div key={m.label} className="bg-[#05080F] p-7 flex flex-col gap-2">
