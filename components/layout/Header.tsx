...
const PRODUCTS = [
  { icon: '⇄',  name: 'Crypto Exchange',      desc: 'CEX & DEX — 100K+ TPS engine',         href: '/products/crypto-exchange',  badge: 'Popular' },
  ...
  { icon: '⚡',  name: 'Trading Bots',         desc: 'Algo trading — 30+ exchanges',          href: '/products/trading-bots' },
  ...
  { icon: '◻',  name: 'Enterprise Wallet',    desc: 'MPC custody — 50+ blockchains',         href: '/products/wallet' },
  { icon: '▦',  name: 'Broker CRM',           desc: 'IB tree, commissions & KYC',            href: '/products/broker-crm' },
  { icon: '☁',  name: 'Cloud & VPS',          desc: 'Bare-metal for financial workloads',    href: '/products/cloud' },
];
...
export default function Header() {
  ...
  return (
    <div className="sticky top-0 z-50">
      ...
      {/* ── Announcement bar ── */}
      {!announceClosed && (
        <div className="bg-[#006FEE] text-white text-center py-2 px-4 text-[12.5px] font-medium flex items-center justify-center gap-2 relative">
          <Zap size={12} className="flex-shrink-0" />
          <span>New: AI Agent Platform 2.0 — autonomous compliance & trading workflows.</span>
          <Link href="/products/ai-agents" className="underline underline-offset-2 font-semibold hover:opacity-80 transition-opacity ml-1">
            Learn more →
          </Link>
      ...
      <header className={`transition-all duration-300 ${
        scrolled
          ? 'bg-white/[0.97] backdrop-blur-2xl border-b border-slate-200/80 shadow-[0_2px_20px_rgba(0,0,0,0.06)]'
          ...
          <div className="flex items-center justify-between h-[62px]">
            ...
            <nav className="hidden xl:flex items-center">
              <NavDrop label="Products" open={activeMenu === 'products'}
                ...
                <Panel onEnter={stayOpen} onLeave={closeMenu} className="w-[720px] -translate-x-[55%]">
                  <div className="p-5">
                    <div className="flex items-center justify-between mb-4 px-1">
                      <span className="text-[10px] font-bold tracking-widest text-slate-400 uppercase">Platform</span>
                      <Link href="/products/crypto-exchange" className="text-[12px] text-[#006FEE] hover:underline font-semibold flex items-center gap-1">
                    ...
                    <div className="grid grid-cols-2 gap-0.5">
                      {PRODUCTS.map(p => (
                        <Link key={p.href} href={p.href}
                          ...
                          <div className="w-9 h-9 rounded-lg bg-slate-100 group-hover:bg-blue-50 flex items-center justify-center text-[16px] flex-shrink-0 transition-colors">
                            {p.icon}
                          </div>
                  ...
                  <div className="p-2">
                    ...
                    {SERVICES.map(s => (
                      <Link key={s.href} href={s.href}
                        ...
                        <ArrowRight size={11} className="text-slate-300 group-hover:text-[#006FEE] flex-shrink-0 transition-colors" />
                      </Link>
                    ))}
                  </div>
                </Panel>
            ...
            <div className="hidden xl:flex items-center gap-1.5">
              <Link href="/contact" className="px-4 py-2 text-[13.5px] font-medium text-slate-500 hover:text-slate-900 rounded-lg hover:bg-slate-50 transition-colors">
                Contact
              </Link>
                    ...
                    <div className="pb-2 pl-2 space-y-0">
                      {item.items.map(sub => (
                        <Link key={sub.href} href={sub.href}
                          className="block py-2 text-[13.5px] text-slate-500 hover:text-[#006FEE] border-b border-slate-50 last:border-0 transition-colors"
...
function Panel({ children, onEnter, onLeave, className }: {
  ...
  return (
    <div className={`absolute top-full pt-2 z-50 ${className}`} onMouseEnter={onEnter} onMouseLeave={onLeave}>
      <div className="bg-white border border-slate-200/80 rounded-2xl shadow-[0_12px_48px_rgba(0,0,0,0.1),0_2px_8px_rgba(0,0,0,0.04)] overflow-hidden">
        {children}
      </div>
    </div>
  );
