      ...
      <section className="section bg-[#020B18]">
        <div className="container">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h2 className="h2 text-white">Choose Your <span className="gt-blue">Partner Level</span></h2>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            {TIERS.map((tier) => (
              <div key={tier.name}
                className={`card rounded-3xl p-7 border flex flex-col ${tier.featured ? 'border-[#6644FF]/40 relative overflow-hidden' : 'border-[#0E1E34]'}`}
                style={{ background: tier.featured ? `${tier.color}06` : undefined }}>
                {tier.featured && (
                  <div className="absolute top-0 right-0 px-4 py-1.5 text-[11px] font-bold text-white rounded-bl-2xl"
                    style={{ background: tier.color }}>
                    MOST POPULAR
                  </div>
                )}
                <div className="text-4xl mb-3">{tier.icon}</div>
                <h3 className="text-[20px] font-bold text-white mb-1">{tier.name}</h3>
                <div className="text-[32px] font-black mb-2" style={{ color: tier.color }}>{tier.commission}</div>
                <p className="text-[12px] text-[#3D5470] mb-1">commission rate</p>
                <p className="text-[13px] text-[#7A8FA6] leading-relaxed mb-5">{tier.desc}</p>

                <div className="space-y-2 mb-5 flex-1">
                  {tier.benefits.map(b => (
                    <div key={b} className="flex items-center gap-2">
                      <CheckCircle size={13} style={{ color: tier.color }} className="flex-shrink-0" />
                      <span className="text-[13px] text-[#7A8FA6]">{b}</span>
                    </div>
                  ))}
                </div>

                <div className="pt-4 border-t border-[#0E1E34] mb-5">
                  <p className="text-[11px] text-[#3D5470] uppercase tracking-wider mb-2">Requirements</p>
                  {tier.requirements.map(r => (
                    <div key={r} className="flex items-center gap-2 text-[12px] text-[#3D5470]">
      ...
      <section className="section-sm bg-[#000008]">
        <div className="container">
          <div className="text-center max-w-2xl mx-auto mb-10">
            <h2 className="h3 text-white">Why Partners Choose <span className="gt-blue">Shivacha</span></h2>
          </div>
          <div className="grid sm:grid-cols-2 md:grid-cols-3 gap-5">
            {PARTNER_BENEFITS.map((b) => (
              <div key={b.title} className="card rounded-2xl p-5 border border-[#0E1E34] hover:border-[#1E3A5A]">
                <div className="text-3xl mb-3">{b.icon}</div>
                <h3 className="text-[15px] font-bold text-white mb-2">{b.title}</h3>
                <p className="text-[13px] text-[#7A8FA6] leading-relaxed">{b.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="section bg-[#020B18]">
        <div className="container">
          <div className="text-center mb-10">
            <h2 className="h3 text-white">Our <span className="gt-blue">Partner Ecosystem</span></h2>
          </div>
          <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-3 mb-10">
            {CURRENT_PARTNERS.map((p) => (
              <div key={p.name} className="card rounded-xl p-4 border border-[#0E1E34] hover:border-[#1E3A5A]">
                <p className="text-[14px] font-semibold text-white mb-1">{p.name}</p>
                <div className="flex items-center justify-between">
                  <span className="badge badge-blue text-[10px]">{p.type}</span>
                  <span className="text-[11px] text-[#3D5470]">{p.region}</span>
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Apply CTA */}
          <div className="glass rounded-3xl p-10 border border-[#0E1E34] text-center max-w-2xl mx-auto">
            <div className="text-4xl mb-4">🚀</div>
            <h3 className="text-[22px] font-bold text-white mb-3">Ready to Partner with Shivacha?</h3>
