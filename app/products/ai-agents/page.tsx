            ...
            <div className="hidden lg:block">
              <div className="dashboard-preview rounded-2xl relative">
                ...
                <div className="dashboard-bar">
                  ...
                  <span className="ml-3 text-[11px] text-[#3D5470]">agents.yourbrand.com — Workflow Monitor</span>
                </div>
                <div className="p-5 space-y-3">
                  ...
                  <div className="flex gap-2 overflow-hidden">
                    {[['Compliance Bot','Running','text-green-400'],['Trade Signal','Running','text-green-400'],['Doc Processor','Queued','text-yellow-400']].map(([name, status, cls]) => (
                      <div key={name} className="glass rounded-lg px-3 py-2 flex-shrink-0">
                        <p className="text-[9px] text-[#3D5470]">Agent</p>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="col-span-2 glass rounded-xl p-3 h-36">
                      <p className="text-[10px] text-[#3D5470] uppercase tracking-wider mb-2">Live Agent Trace — Compliance Bot</p>
                      <div className="space-y-1.5">
                        {[['→ Fetch transactions (DB Tool)','12ms','✓'],['→ Screen vs OFAC (API Tool)','38ms','✓'],['→ Analyze patterns (Claude 3)','410ms','✓'],['→ Human review gate','Waiting','⏳'],['→ File SAR (API Tool)','—','—']].map(([step, t, status]) => (
                          <div key={step} className="flex justify-between text-[9px]">
                            <span className="text-[#7A8FA6] truncate w-40">{step}</span>
                            <span className="text-[#3D5470]">{t}</span>
                            <span className={status === '✓' ? 'text-green-400' : status === '⏳' ? 'text-yellow-400' : 'text-[#3D5470]'}>{status}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="glass rounded-xl p-3 h-36 flex flex-col justify-between">
                      <p className="text-[10px] text-[#3D5470] uppercase tracking-wider">Model Usage</p>
                      <div className="space-y-1.5">
                        {[['GPT-4o',62],['Claude 3',28],['Gemini',10]].map(([m, pct]) => (
                          <div key={m as string}>
                            <div className="flex justify-between text-[9px] mb-0.5">
                              <span className="text-[#7A8FA6]">{m}</span>
      ...
      <section className="section bg-[#020B18]">
        <div className="container">
          <div className="text-center max-w-2xl mx-auto mb-14">
            ...
            <h2 className="h2 text-white mb-4">Everything to Build & Deploy <span style={{ color: '#00D4FF' }}>Enterprise AI Agents</span></h2>
            <p className="lead">One platform. Design, test, deploy, and monitor autonomous AI workflows at enterprise scale.</p>
          </div>
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map(f => (
              <div key={f.title} className="card rounded-2xl p-6 border border-[#0E1E34] hover:border-[#00D4FF]/30 group">
                <div className="text-4xl mb-4">{f.icon}</div>
                <h3 className="text-[16px] font-bold text-white mb-2 group-hover:text-[#00D4FF] transition-colors">{f.title}</h3>
      ...
      <section className="section bg-[#000008]">
        <div className="container">
          <div className="text-center max-w-2xl mx-auto mb-14">
            ...
            <h2 className="h2 text-white mb-4">Agents Built for <span style={{ color: '#00D4FF' }}>Financial Services</span></h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {USE_CASES.map(u => (
              <div key={u.title} className="card rounded-2xl p-6 border border-[#0E1E34] hover:border-[#00D4FF]/25 group">
                <div className="text-3xl mb-3">{u.icon}</div>
                <h3 className="text-[15px] font-bold text-white mb-2 group-hover:text-[#00D4FF] transition-colors">{u.title}</h3>
        ...
        <div className="container">
          <div className="grid lg:grid-cols-2 gap-16 items-start">
            <div>
              ...
              <h2 className="h2 text-white mb-6">Built for <span style={{ color: '#00D4FF' }}>Enterprise Scale</span></h2>
            ...
            <div>
              ...
              <h3 className="h3 text-white mb-6">The Stack Behind the Agents</h3>
              ...
              <div className="glass-lg rounded-2xl p-6 border border-[#0E1E34]">
                <p className="text-[12px] text-[#3D5470] uppercase tracking-wider mb-4">Platform Performance</p>
      ...
      <section className="section bg-[#000008]">
        <div className="container">
          <div className="text-center max-w-2xl mx-auto mb-14">
            ...
            <h2 className="h2 text-white mb-4">From Idea to <span style={{ color: '#00D4FF' }}>Autonomous Agent</span></h2>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {STEPS.map((s, i) => (
              <div key={s.n} className="card rounded-2xl p-6 border border-[#0E1E34] relative group hover:border-[#00D4FF]/30">
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[['82%','Reduction in Compliance Review Time'],['$2.1M','Annual Savings in Operations'],['14','Agents Deployed in 6 Weeks'],['67%','Fewer False-Positive Escalations'],['100%','Audit Trail Coverage'],['Zero','Regulatory Findings — Year One']].map(([v, l]) => (
                <div key={l} className="glass-lg rounded-2xl p-6 border border-[#0E1E34] hover:border-[#00D4FF]/25 transition-colors">
      ...
      <section className="section bg-[#000008]">
        <div className="container">
          <div className="text-center max-w-2xl mx-auto mb-12">
            ...
            <h2 className="h2 text-white mb-4">100+ <span style={{ color: '#00D4FF' }}>Pre-Built Connectors</span></h2>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
            {['Salesforce CRM','SAP ERP','Bloomberg Terminal','Refinitiv Eikon','Interactive Brokers','FIX Protocol 4.4','Slack Notifications','Microsoft Teams','ServiceNow','JIRA','PostgreSQL','Snowflake','Pinecone','Weaviate','Twilio SMS','SendGrid Email'].map(name => (
              <div key={name} className="flex items-center gap-2.5 p-3 rounded-xl glass border border-[#0E1E34] hover:border-[#00D4FF]/25 transition-colors">
